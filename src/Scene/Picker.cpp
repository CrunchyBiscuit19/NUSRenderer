...
Picker::Picker(Renderer* renderer) : mRenderer(renderer), mDrawPipelineLayout(nullptr), mPickPipelineLayout(nullptr), mDescriptorSet(nullptr), mDescriptorSetLayout(nullptr), mImguizmoOperation(ImGuizmo::TRANSLATE), mClickedInstance(nullptr) {}
...
void Picker::initBuffer() {
    mBuffer = mRenderer->mResources.createBuffer(sizeof(PickerData), vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress, VMA_MEMORY_USAGE_CPU_TO_GPU);
...
void Picker::initImage() {
    mImage = mRenderer->mResources.createImage(mRenderer->mInfrastructure.mDrawImage.imageExtent,
    ...
    mRenderer->mImmSubmit.mCallbacks.emplace_back([this](Renderer* renderer, vk::CommandBuffer cmd) {
        vkhelper::transitionImage(cmd, *mImage.image, vk::PipelineStageFlagBits2::eNone, vk::AccessFlagBits2::eNone, vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eUndefined,
...
void Picker::initDrawPipeline() {
    ...
    mDrawPipelineBundle = PipelineBundle(mRenderer->mInfrastructure.mLatestPipelineId++, drawPipelineBuilder.buildPipeline(mRenderer->mCore.mDevice), *mDrawPipelineLayout);
...
void Picker::initPickPipeline() {
    ...
    mPickPipelineBundle = PipelineBundle(mRenderer->mInfrastructure.mLatestPipelineId++, pickPipelineBuilder.buildPipeline(mRenderer->mCore.mDevice), *mPickPipelineLayout);
...
void Picker::imguizmoFrame() const {
    ...
    ImGuizmo::SetRect(0, 0, static_cast<float>(mRenderer->mInfrastructure.mDrawImage.imageExtent.width), static_cast<float>(mRenderer->mInfrastructure.mDrawImage.imageExtent.height));
