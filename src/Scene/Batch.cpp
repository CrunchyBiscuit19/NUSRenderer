...
Batch::Batch(Renderer* renderer, Primitive& primitive, int pipelineId) {
    ...
    renderItemsBuffer = renderer->mResources.createAddressedBuffer(
        MAX_RENDER_ITEMS * sizeof(RenderItem), vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eIndirectBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress, VMA_MEMORY_USAGE_GPU_ONLY);
    renderer->mCore.labelResourceDebug(renderItemsBuffer.buffer, fmt::format("RenderItemsBuffer{}", pipelineId).c_str());
    ...
    renderInstancesBuffer =
        renderer->mResources.createAddressedBuffer(MAX_RENDER_ITEMS * sizeof(RenderInstance), vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress, VMA_MEMORY_USAGE_GPU_ONLY);
